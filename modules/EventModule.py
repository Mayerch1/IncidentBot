...
class EventModule(commands.Cog):
    ...
    async def setup(self, cmd, mode):
        ...
        if mode:
            if mode[0] == 'category':
                ...
                await cmd.send(EventModule.SETUP_HELP)
    ...
    async def incident_cmd(self, cmd, *mode):
        ...
        if not mode or mode[0] == 'create':
            ...
            await cmd.send(EventModule.INC_HELP)


def setup(client):
    client.add_cog(EventModule(client))
